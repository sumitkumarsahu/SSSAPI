...
def readconf(input0):
    ...
    if UTILITY_HOSTNAME is None:
        print("-- [ERROR] Utility hostname should be provided inside apimgr.yml file... --")
        sys.exit(1)
    ...
    if IMAGE_NAME is None:
        print("-- [ERROR] Image name should be provided inside apimgr.yml file... --")
        sys.exit(1)
    ...
    if IMAGE_VERSION is None:
        print("-- [ERROR] Image version should be provided inside apimgr.yml file... --")
        sys.exit(1)
    ...
    # ESS API Server SSH Port, default 20022
...
def checkdir():
    ...
    if not (os.path.isdir(cfg["CONTAINER"]["BUILDS"])):
        os.makedirs(cfg["CONTAINER"]["BUILDS"])
        print("-- [INFO] Builds directory does not exist, created now --")


# -----------------------------------------------------------------------------
# Depreciation warning
# -----------------------------------------------------------------------------
def apimgr_EOL_warning():
    ALLOW_APIMGR = os.getenv('ALLOW_APIMGR')
    if ALLOW_APIMGR is None:
        print("-- [ERROR] You cannot run apimgr directly, use startAPIcont instead")
        print("           If you still want to run apimgr directly, set ALLOW_APIMGR to value 1 on the shell")
        print("           but be aware this is not supported and any error you encounter you need to run with startAPIcont")
        time.sleep(2)
        sys.exit(1)
    elif ALLOW_APIMGR == "1":
        print(
            "-- [WARNING] You should not run apimgr directly, use startAPIcont instead")
        print(
            "-- [WARNING] apimgr it is going to be depreciated in a future release")
        print(
            "-- [WARNING] Be aware that any error you encounter you need to reproduce running startAPIcont")
        time.sleep(5)
    else:
        print("-- [ERROR] You cannot run apimgr directly, use startAPIcont instead")
        print("           If you still want to run apimgr directly, set ALLOW_APIMGR to value 1 on the shell")
        print("           Now it has a value but it is not set to '1'.")
        print("           Be aware this is not supported and any error you encounter you need to run with startAPIcont")
        time.sleep(2)
        sys.exit(1)
...
# Run Container
# -----------------------------------------------------------------------------
def run_container(force, is_startapicont=False):
    global UTILITY_HOSTNAME
    global API_PORT
    global SSH_PORT
    global IMAGE_NAME
    ...
    if not is_startapicont:
        apimgr_EOL_warning()

    # not needed anymore since we changed the port for http within container
    # rc += guiAndNftable()
    ...
    if (returned_output != "".encode()):
        ...
        cmd = "podman create --privileged --syslog" + \
            ...
            " -v " + cfg["CONTAINER"]["BUILDS"] + ":/home/essbuilds/" + \
        ...
        container_ports_details = " -p " + API_PORT + ":" + "46443" + "/tcp" + \
            " -p " + SSH_PORT + ":" + "22" + "/tcp"
        ...
        print("-- [INFO] Setting SSS API Container to start as a systemd service --")
        print("-- [INFO] SSS API Container set to autostart --")
        ...
        cmd += "loginctl enable-linger apiadmin ; "
...
def install_image_from_file(image_file_name, force):
    ...
    # img_tag=$(echo "Loaded image: cp.stg.icr.io/cp/scalesystem/sss_api:6.2.3.0" | grep "Loaded image" | awk -F": " '{print $2}')
...
def tag_image(IMAGE_ID):
    rc = 0
    if IMAGE_NAME is not None and IMAGE_VERSION is not None:
        print("-- [INFO] Adding TAG name: " + IMAGE_NAME +":" + IMAGE_VERSION + " to image id " + IMAGE_ID + "--")
        cmd = "podman image tag " + IMAGE_NAME + ":" + IMAGE_VERSION + " " + IMAGE_ID
        _rc = subprocess.call(cmd, shell=True)
        if _rc == 0:
            print(
                "-- [INFO] Added tag " +
...
def install_image_from_repo(force):
    ...
    print("-- [INFO] Goint to pull Deployment GUI Container image from IBM repository... --")
...
# -----------------------------------------------------------------------------
# Defect - 315692 - nftables needs to be restarted and gui stopped before container starts
# -----------------------------------------------------------------------------
def guiAndNftable():
    rc = 0
    cmd = "systemctl is-active gpfsgui "
    guiStatus = subprocess.call(cmd, shell=True)
    if guiStatus == 0:
        print(
        "-- [ERROR] gpfsgui service is active, please stop it before starting container --")
        rc += 1
        sys.exit(rc)
    ...
    if rc != 0:
        print(
            "-- [ERROR] Unable to restart nftables service --")
    return rc
...
def main():
    ...
    parser = argparse.ArgumentParser(
        epilog='This script run API container on IBM Utility host to server ESS API. ')

    parser.add_argument('-c', '--config', action='store',
                        default="apimgr.yml", dest='config_file',
                        required=False,
    ...
    parser.add_argument('-net', '--network-name', action='store',
                        default="api_network", dest='network_name',
                        required=False,
                        help='Creates podman CNI network with default name api_network.')

    mutual_group.add_argument('-r', '--run', action='store_true',
                              default=False, dest='run',
                              required=False,
                              help='Runs SSS API Container.')
    ...
    if allow_apimgr is None:
        print(
            "-- [WARNING] The tool 'apimgr' has been deprecated. " +
            "Please use 'startAPIcont' instead. --"
        )
        sys.exit(0)
    elif allow_apimgr == "1":
        print(
            "-- [WARNING] The tool 'apimgr' has been deprecated. " +
            "Proceeding to use apimgr since you are focing it by export ALLOW_APIMGR=1. " +
    ...
    if input0.install:
        if (input0.image_file_name is None and IMAGE_NAME is None):
            print("-- [ERROR] Image tarball name using -f option or IMAGE_NAME should be inside apimgr.yml")
            sys.exit(1)
