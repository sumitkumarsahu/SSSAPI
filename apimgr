...
def run_container(force, is_startapicont=False):
    ...
    if (returned_output != "".encode()):
        ...
        cmd = "podman cp " + credentials_file + " utilityBareMetal:/var/lib/ess/" + credentials_file

        rc = subprocess.call(cmd, shell=True)
        if rc != 0:
            print("-- [ERROR] Failed to copy credentials.properties into container --")
            print("-- Exiting... --")
            sys.exit(rc)
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
            print(f"-- [WARNING] {file_path} has been deleted.")
        else:
...
def install_image_from_file(image_file_name, force):
    ...
    # img_tag=$(echo "Loaded image: cp.icr.io/cp/scalesystem/sss_api:6.2.3.0" | grep "Loaded image" | awk -F": " '{print $2}')
